...
- **Founder narrative:** `dominantStage`, **`worstMetricGroup`** (strongest constraint axis), **`contributingSignals`**, overall health, and revenue risk feed **`buildExecutiveNarrativeContext`** (`lib/executiveNarrativeContext.ts`) → **`generateExecutiveSummaryParagraph`** (`lib/executiveSummaryParagraphOpenAI.ts`). They are **not** rendered as standalone UI.
...
**Locations:**
- `lib/executiveNarrativeContext.ts` — **`getPerformanceNarrativeMode`**, **`NARRATIVE_RULES`**, **`buildExecutiveNarrativeContext`**, **`validateNarrativeConsistency`**: system chooses **POSITIVE / BALANCED / NEGATIVE** from overall health (0–10) + revenue exposure (LOW/MEDIUM/HIGH from revenue risk score); strengths/weaknesses from dashboard metric verdicts where available.
- `lib/aiExecutiveSummary.ts` — **`generateExecutiveSummaryJson`**: deterministic four-part facts (`headline`, `impact`, `constraint`, `action`) used as **fallback** when the LLM output fails validation and as **`executive_summary_json`** for structured consumers. Headline/impact lines are **mode-aware** (same narrative mode as the paragraph path when `overallHealth100` + `revenueRiskScore` are passed in options).
- `lib/executiveSummaryParagraphOpenAI.ts` — **`buildFounderExecutiveInputsFromAnalyzeData`** remains for legacy callers. **`generateExecutiveSummaryParagraph`** takes **`ExecutiveNarrativeContext`**, uses a **structured writing** system/user prompt (no free-form “analyze this”), **`gpt-4o-mini`**, **180–220 words**, duplicate-sentence + banned-phrase filters, and **`validateNarrativeConsistency`** (with **one** combined repair retry). Output normalization includes quote/ascii cleanup. **`summary.executive_summary`** and **`summary.executive_summary_paragraph`** are the same string. With **`EXEC_SUMMARY_DEBUG=1`**, **`summary.executive_summary_debug`** includes **`RAW_AI_SUMMARY`** vs **`FINAL_SUMMARY`** and **`usedFallback`**.
- **Dashboard:** `app/dashboard/[projectId]/page.tsx` — **Executive summary** section (`#executive-summary`) is the **only narrative surface** (the old standalone "What to fix first / System Diagnosis" block was removed). Inner container uses **`max-w-[min(92rem,calc(100vw-2rem))]`** for a full-width horizontal reading strip (no 88ch cap). **`text-pretty` removed** from the paragraph wrapper (Chrome's "pretty" wrapping shrinks the effective column to balance lines, which was clipping long summaries to ~380px wide). Typography unchanged: `text-[1.05rem] sm:text-[1.12rem]`, line-height `1.8` / `sm:1.82`, tracking `0.01em`, color `text-slate-100/95`.
